...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    for i in range(len(cm)):
        F1 = 0.
        divisor = np.sum(cm[i, :]) + np.sum(cm[:, i])
        if divisor != 0:
            F1 = 2. * cm[i, i] / divisor
...
def show_results(results, vis, label_values=None, aggregated=False, df_column_entry_dict=None):
    text = ""
...
def print_memory_metrics(msg, df_dict):
    if df_dict is not None:
        df_dict['GPU Memory allocated at '+msg+' [Bytes]']      = torch.cuda.memory_allocated()
        df_dict['GPU Max memory allocated at '+msg+' [Bytes]']  = torch.cuda.max_memory_allocated()
        df_dict['GPU Memory cached at '+msg+' [Bytes]']         = torch.cuda.memory_cached()
        df_dict['GPU Max memory cached at '+msg+' [Bytes]']     = torch.cuda.max_memory_cached()
        print("\n--- Printing GPU memory metrics at the event: "+msg+" ---")
        print("GPU Memory allocated:    \t"+str("{:,}".format(df_dict['GPU Memory allocated at '+msg+' [Bytes]']))    + " Bytes")
        print("GPU Max memory allocated:\t"+str("{:,}".format(df_dict['GPU Max memory allocated at '+msg+' [Bytes]']))+ " Bytes")
        print("GPU Memory cached:       \t"+str("{:,}".format(df_dict['GPU Memory cached at '+msg+' [Bytes]']))       + " Bytes")
        print("GPU Max memory cached:   \t"+str("{:,}".format(df_dict['GPU Max memory cached at '+msg+' [Bytes]']))   + " Bytes")
        print("--- End of GPU memory metrics measurement ---\n")
    else:
        print("\n--- Printing GPU memory metrics at the event: "+msg+" ---")
        print("GPU Memory allocated:    \t"+str("{:,}".format(torch.cuda.memory_allocated()))    + " Bytes")
        print("GPU Max memory allocated:\t"+str("{:,}".format(torch.cuda.max_memory_allocated()))+ " Bytes")
        print("GPU Memory cached:       \t"+str("{:,}".format(torch.cuda.memory_cached()))       + " Bytes")
        print("GPU Max memory cached:   \t"+str("{:,}".format(torch.cuda.max_memory_cached()))   + " Bytes")
        print("--- End of GPU memory metrics measurement ---\n")

def start_mem_measurement():
    tracemalloc.start()

def stop_mem_measurement(event, df_dict):
    print("\n--- Printing RAM memory metrics at the event: "+event+" ---")
    current, peak = tracemalloc.get_traced_memory()
    df_dict['Current RAM usage at '+event+' [Bytes]'] = current
    df_dict['Peak RAM usage at '+event+' [Bytes]'] = peak
    print("Current RAM usage: {:,}".format(df_dict['Current RAM usage at '+event+' [Bytes]']) + " Bytes")
    print("Peak RAM usage: {:,}".format(df_dict['Peak RAM usage at '+event+' [Bytes]']) + " Bytes")
    tracemalloc.stop()
    print("--- End of RAM memory metrics measurement ---\n")