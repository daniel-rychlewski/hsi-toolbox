...
from DeepHyperX.custom_models import Roy19, Santara16, Cao17, LuoRNN
from DeepHyperX.utils import grouper, sliding_window, count_sliding_window, \
    camel_to_snake, print_memory_metrics


def distiller_wrapper_get_model(name, **kwargs):
...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
        ...
        kwargs.setdefault('epoch', 100)  # suffices for good accuracy, as seen in gpu02 checkpoint accuracies for respective epoch number
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = False
        ...
        print("warning: the chen neural network performs particularly awful, so this might take a long time")
        ...
        epoch = kwargs.setdefault('epoch', 100)  # suffices for good accuracy, as seen in gpu02 checkpoint accuracies for respective epoch number
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        ...
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    # custom models from here on
    elif name == 'roy':
        kwargs.setdefault('epoch', 100)
        lr = kwargs.setdefault('lr', 0.001)
        kwargs.setdefault('batch_size', 256)
        kwargs.setdefault('patch_size', 25) # window size
        center_pixel = True
        model = Roy19(n_bands, n_classes, window_size=kwargs['patch_size'])
        optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-06)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'santara':
        kwargs.setdefault('epoch', 40) # anything else just takes too long. 100 epochs are suggested in the github code, but just no
        lr = kwargs.setdefault('lr', 0.0005)
        kwargs.setdefault('batch_size', 200)
        patch_size = kwargs.setdefault('patch_size', 3)
        center_pixel = True
        block1_conv1 = 3330 # 3333
        n_bands2 = 10
        assert (block1_conv1 // n_bands2) == int(block1_conv1 / n_bands2)
        model = Santara16(n_channels=n_bands, block1_conv1=block1_conv1, n_bands=n_bands2, patch_size=patch_size, n_classes=n_classes)
        optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=5e-9)
        criterion = nn.NLLLoss(weight=kwargs['weights'])
    elif name == 'cao':
        # training_iters set to 10000 in the original -> absolutely not feasible in practice
        lr = kwargs.setdefault('lr', 0.001)
        kwargs.setdefault('batch_size', 100)
        patch_size = kwargs.setdefault('patch_size', 9)
        center_pixel = True
        model = Cao17(patch_size=patch_size, num_band=n_bands, num_classes=n_classes)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'luo_rnn':
        # n_iter = 3000 in original absolutely not feasible, so leave epochs at default value
        lr = kwargs.setdefault('lr', 0.001)
        batch_size = kwargs.setdefault('batch_size', 512*2*8)#512
        patch_size = kwargs.setdefault('patch_size', 5)
        center_pixel = True
        model = LuoRNN(n_bands=n_bands, window_size=patch_size, n_filter=16, m_filter=16, hidden_size=128, n_time=5, n_classes=n_classes, batch_size=batch_size)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    else:
    ...
    epoch = epoch // kwargs['speedup'] # speedup
...
class HamidaEtAl(nn.Module):
    ...
    def forward(self, x):
        ...
        try:
            ...
            # model has been pruned - only then determine new shape, for performance reasons
            x = x.view(-1, self._get_final_flattened_size())
        x = self.fc(x)
        return x
...
@deprecated(reason="spectacularly awful performance, use other networks")
...
class LuoEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=3, n_planes=90):
        ...
        self.conv1 = nn.Conv3d(1, self.n_planes, (24, 3, 3), padding=0, stride=(9,1,1))
    ...
    def forward(self, x):
        ...
        # self.n_planes = self.conv1.in_channels  # necessary adjustment for next test batch because this number is varied dynamically by distillers channel pruning

        x = F.relu(self.conv1(x))
        b = x.size(0)
        # self.n_planes = self.conv1.out_channels # only uncomment for distiller pruning. for quantization, keep this out, otherwise: RangeLinearQuantParamLayerWrapper object has no attribute out_channels
        x = x.view(b, 1, -1, self.n_planes)
        x = F.relu(self.conv2(x))
        try:
            x = x.view(-1, self.features_size)
        except RuntimeError:
            # for pruned tensors, occurs for distiller channel-based pruning.
            # number of incoming channels of linear layer absolutely must be adjusted now because they did change after a pruning step
            self.features_size = self._get_final_flattened_size()
            # self.fc1.in_features = self.features_size
            self.fc1 = nn.Linear(self.features_size, 1024)
            x = x.view(-1, self.features_size)
        x = F.relu(self.fc1(x))
        x = self.fc2(x)
...
class BoulchEtAl(nn.Module):
    ...
    # https://stackoverflow.com/questions/55033669/encoding-and-decoding-pictures-pytorch
    def encode(self, X):
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
    ...
    save_epoch = epoch # save only the last epoch's weights. saves a lot of storage, the rest is unnecessary. # epoch // 20 if epoch > 20 else 1
    ...
    path = None # will be set when saving the model
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        print_memory_metrics("epoch "+str(e)+" of training started", None)
        ...
        # Save the weights
        if e % save_epoch == 0:
            path = save_model(net, camel_to_snake(str(net.__class__.__name__)), data_loader.dataset.name, epoch=e, metric=abs(metric))
    return path

# https://github.com/L1aoXingyu/pytorch-beginner/blob/master/08-AutoEncoder/simple_autoencoder.py
def autoencoder_train(net, optimizer, loader, num_epochs):
    save_epoch = num_epochs-1 # save space
    path = None
    compressed_X = []
    for epoch in range(num_epochs):
        # for data, (_, target) in (zip(loader.dataset.data, loader)):
        for data in loader.dataset.data:
            data = torch.tensor(data)
            # img, _ = data
            # img = img.view(img.size(0), -1)
            from torch.autograd import Variable
            img = Variable(torch.tensor(data)).cuda()
            # ===================forward=====================
            output = net(img)
            # ===================backward====================
            optimizer.zero_grad()
            # loss = optimizer(data, target)
            # loss[0]().backward()
            optimizer.step()
        # ===================log========================
        print('epoch [{}/{}]'.format(epoch + 1, num_epochs)) #, loss:{:.4f} #, loss.data[0])
        # Save the weights
        if epoch % save_epoch == 0:
            path = save_model(net, camel_to_snake(str(net.__class__.__name__)), loader.dataset.name, epoch=epoch)

    for data in loader.dataset.data:
        from torch.autograd import Variable
        img = Variable(torch.tensor(data)).cuda()
        compressed_X.append(net.encode(img.unsqueeze(1)).detach().cpu().numpy()) # only add once, not number-of-epochs times

    compressed_X = np.array(compressed_X)
    # e.g. (100,100,3,1) for JasperRidge-198 -> (100,100,3). Needs to be converted back for decoding
    compressed_X = compressed_X.reshape((compressed_X.shape[0], compressed_X.shape[1], compressed_X.shape[2]))

    return path, compressed_X

def save_model(model, model_name, dataset_name, **kwargs):
    """
    Saves either the model parameters or its weights to the checkpoints-subfolder, depending on the class of the parameter "model".
    :return full path to the saved file
    """
    from DeepHyperX.batch import STORE_EXPERIMENT_LOCATION
    model_dir = STORE_EXPERIMENT_LOCATION + model_name + "/" + dataset_name + "/"
    ...
    if isinstance(model, torch.nn.Module):
        ...
        try:
            ...
            filename = str(datetime.datetime.now()).replace(':', '-') + "_epoch{epoch}".format(**kwargs)
        tqdm.write("\nSaving neural network weights in {}".format(filename))
        path = model_dir + filename + '.pth'
        torch.save(model.state_dict(), path)
        return path
    else:
        filename = str(datetime.datetime.now()).replace(':','-')
        tqdm.write("\nSaving model params in {}".format(filename))
        path = model_dir + filename + '.pkl'
