...
def sparsity_3D(tensor):
    ...
    if tensor.dim() != 5:#4:
        return 0
    view_3d = tensor.view(-1, tensor.size(1) * tensor.size(2) * tensor.size(3) * tensor.size(4))
...
def sparsity_ch(tensor):
    ...
    view_2d = tensor.view(-1, tensor.size(2) * tensor.size(3) * tensor.size(4))
...
def sparsity_blocks(tensor, block_shape):
    ...
    if len(block_shape) != 5:#4:
        raise ValueError("Block shape must be specified as a 4-element tuple")
    block_repetitions, block_depth, block_height, block_width, block_whatever = block_shape
    ...
    kernel_size = tensor.size(2) * tensor.size(3) * tensor.size(4)
...
def _validate_input_shape(dataset, input_shape, model_name=None):
    if dataset:
        try:
            return tuple(distiller.apputils.classification_get_input_shape(dataset, model_name))
        except ValueError:
...
def get_dummy_input(dataset=None, device=None, input_shape=None, model_name=None):
    ...
    input_shape = _validate_input_shape(dataset, input_shape, model_name)
