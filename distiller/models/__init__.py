...
from DeepHyperX import models as hsi_models
from distiller.apputils import model_to_short
...
# HSI_MODEL_NAMES = sorted(name for name in hsi_models.__dict__ if not name.startswith("__") and callable(hsi_models.__dict__[name]))
HSI_MODEL_NAMES = ['Baseline', 'BoulchEtAl', 'Cao17', 'ChenEtAl', 'HamidaEtAl', 'HeEtAl', 'HuEtAl', 'LeeEtAl', 'LiEtAl', 'LiuEtAl', 'LuoEtAl', 'LuoRNN', 'MouEtAl', 'Roy19', 'Santara16', 'SharmaEtAl']
...
ALL_MODEL_NAMES = sorted(map(lambda s: s.lower(),
                            set(IMAGENET_MODEL_NAMES + CIFAR10_MODEL_NAMES + list(model_to_short[hsimodel.lower()] for hsimodel in HSI_MODEL_NAMES) + MNIST_MODEL_NAMES)))


def create_model(pretrained, dataset, arch, parallel=True, device_ids=None, hyperparams=None):
    ...
    else:  # a hyperspectral dataset -> choose any hyperspectral model
        try:
            # populate the hyperparams on the way
            model, _, _, hyperparams = hsi_models.distiller_wrapper_get_model(arch,
                                                                              hyperparams=hyperparams)
        except KeyError:
            raise ValueError("Hyperspectral model could not be found")
