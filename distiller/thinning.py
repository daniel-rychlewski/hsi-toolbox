...
def remove_filters(model, zeros_mask_dict, arch, dataset, optimizer):
    sgraph = create_graph(dataset, model, arch)
...
def create_thinning_recipe_channels(sgraph, model, zeros_mask_dict):
    ...
    def handle_layer(layer_name, param_name, num_nnz_channels):
        ...
        assert isinstance(layers[layer_name], torch.nn.modules.Conv2d) or isinstance(layers[layer_name], torch.nn.modules.Conv1d) or isinstance(layers[layer_name], torch.nn.modules.Conv3d)
...
def create_thinning_recipe_filters(sgraph, model, zeros_mask_dict):
    ...
    def handle_layer(layer_name, param_name, num_nnz_filters):
        ...
        for successor in successors:
            if isinstance(layers[successor], torch.nn.modules.Conv2d) or isinstance(layers[successor], torch.nn.modules.Conv1d) or isinstance(layers[successor], torch.nn.modules.Conv3d):
                ...
                if ((int(math.sqrt(fm_size))) ** 2 == fm_size):
                    # can safely apply sqrt to split dimensions
                    fm_height = fm_width = int(math.sqrt(fm_size))
                else:
                    fm_height, fm_width = find_two_factors(fm_size)

                # fm_height = fm_width = fm_whatever = int(fm_size ** (1 / 3.0))

                view_4D = (layers[successor].out_features, layers[layer_name].out_channels, fm_height, fm_width)
                # view_4D = (layers[successor].out_features, layers[layer_name].out_channels, fm_height, fm_width, fm_whatever)
    ...
    def find_two_factors(fm_size):
        for i in range(2, fm_size//2):
            if fm_size % i == 0:
                return i, fm_size//i
        return 1, fm_size
    ...
    def handle_conv_successor(thinning_recipe, layers, successor, num_nnz_filters, indices):
        ...
        if layers[successor].groups == 1:
            ...
            if num_nnz_filters <= layers[successor].out_channels:
    ...
    for layer_name, param_name, param in sgraph.named_params_layers():
        ...
        if param.dim() not in [4,5]: # todo expand for 5. Result: impossible to do in 6 months time frame
            continue
