...
def conv_visitor(self, input, output, df, model, memo):
    ...
    elif isinstance(self, torch.nn.Conv1d):
        macs = (distiller.volume(output) *
            (self.in_channels / self.groups * self.kernel_size[0]))
    elif isinstance(self, torch.nn.Conv3d):
        macs = (distiller.volume(output) *
            (self.in_channels / self.groups * self.kernel_size[0] * self.kernel_size[1] + self.kernel_size[2]))
