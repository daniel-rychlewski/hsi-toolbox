...
def group_threshold_mask(param, group_type, threshold, threshold_criteria, binary_map=None):
    ...
    if group_type == '2D':
        ...
        if len(param.shape) == 5:
            a = binary_map.expand(param.size(2) * param.size(3) * param.size(4),
                                  param.size(0) * param.size(1)).t()
            return a.view(param.size(0), param.size(1), param.size(2), param.size(3), param.size(4)), binary_map
        else: # usually == 4
        ...
        if binary_map is None:
            binary_map = group_threshold_binary_map(param, group_type, threshold, threshold_criteria)
        if len(param.shape) == 5:
            a = binary_map.expand(param.size(1) * param.size(2) * param.size(3) * param.size(4), param.size(0)).t()
        else: # usually == 4
        ...
        c = a.unsqueeze(-1)
        if len(param.shape) == 5:
            d = c.expand(num_filters, num_kernels_per_filter, param.size(2) * param.size(3) * param.size(4)).contiguous()
            return d.view(param.size(0), param.size(1), param.size(2) * param.size(3), param.size(4)), binary_map
        else:  # usually == 4
