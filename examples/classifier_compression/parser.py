...
def get_parser():
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', default='resnet18', type=lambda s: s.lower(),
                        ...
                        ' (default: resnet18)')
    parser.add_argument('-d', '--dataset', help='choose dataset: IndianPines, SalinasA, PaviaC etc.')
    ...
    parser.add_argument('--cuda', help='specify cuda device') # todo might not be necessary - replace with --gpus ?

    # taken from DeepHyperX main.py
    parser.add_argument('--formerly_used_technique',
                        help="band selection technique previously applied on the model for the checkpoint that is being read in now",
                        default=None)
    parser.add_argument('--old_n_components',
                        help="n_components for band selection technique previously applied on the model for the checkpoint that is being read in now",
                        default=None)
