...
from DeepHyperX.batch import DISTILLER_PATH
from DeepHyperX.utils import start_mem_measurement, stop_mem_measurement, print_memory_metrics
...
def main():
    ...
    hyperparams.update({'model': args.arch}) # for load_data, get_model needs the model name so that hyperparams can be populated prior to retrieving dataset
    if args.formerly_used_technique is not None:
        hyperparams.update({'formerly_used_technique': args.formerly_used_technique})
    ...
    model = create_model(args.pretrained, args.dataset, args.arch,
                         parallel=not args.load_serialized, device_ids=args.gpus, hyperparams=hyperparams)
...
def test(test_loader, model, criterion, loggers, activations_collectors, args):
    ...
    with collectors_context(activations_collectors["test"]) as collectors:
        ...
        import pandas as pd
        frame = pd.DataFrame([list(df_column_entry_dict.values())], columns=list(df_column_entry_dict.keys()))
        frame.to_excel(DISTILLER_PATH+msglogger.logdir + ".xlsx", index=False)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1):
    ...
    if args.display_confusion:
        if hasattr(args, 'num_classes'):
            ...
            confusion = tnt.ConfusionMeter(args.n_classes)
...
def greedy(model, criterion, optimizer, loggers, args):
    hyperparams = vars(args)
    hyperparams.update({'model':args.model})
    train_loader, val_loader, test_loader, _ = load_data(args, hyperparams=hyperparams)
...
def load_data(args, fixed_subset=False, hyperparams=None):
    return apputils.load_data(args.dataset, os.path.expanduser(args.data), args.batch_size,
                              args.workers, hyperparams, args.validation_split, args.deterministic,
                              args.effective_train_size, args.effective_valid_size, args.effective_test_size,
                              fixed_subset, int(args.cuda))
